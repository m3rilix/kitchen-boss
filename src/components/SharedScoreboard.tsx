...
import { Trophy, Activity, Repeat, Hourglass, Timer, BarChart3 } from 'lucide-react';
      ...
      {order.map((player) => {
        ...
        return (
          <div key={player.id} className="flex flex-col items-center gap-1">
            ...
            <div className={`w-16 ${heightMap[player.name] || 'h-16'} rounded-t-lg border-t border-x flex items-center justify-center ${colorMap[player.name] || 'bg-slate-100 border-slate-300'}`}>
              <RankBadge rank={rank} />
            </div>
          </div>
        );
...
function PlayerRow({ rank, player, partnerName }: { rank: number; player: PlayerWithDiff; partnerName?: string }) {
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-2.5 ${isTop3 ? 'bg-yellow-50/40 dark:bg-yellow-900/5' : ''} ${rank % 2 === 0 ? 'bg-slate-50/60 dark:bg-slate-800/30' : ''}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {partnerName && <p className="text-xs text-slate-400 dark:text-slate-500">with {partnerName}</p>}
      </div>
...
function PairRow({ rank, pair, players }: { rank: number; pair: PairWithDiff; players: Player[] }) {
...
interface SharedScoreboardProps {
  session: Session;
}

export function SharedScoreboard({ session }: SharedScoreboardProps) {
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="px-2 py-0.5 text-xs font-medium text-slate-500 bg-slate-100 dark:bg-slate-700 dark:text-slate-400 rounded-full">
                  Session ended
                </span>
              </div>
      ...
      <main className="container mx-auto px-4 py-6 max-w-lg">
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
          ...
          <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-700 flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">Final Results</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{session.name}</p>
            </div>
          </div>
          ...
          {totalGames === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-slate-400 dark:text-slate-500">
              ...
              <p className="text-sm">No games were played</p>
            </div>
          ) : (
            <>
              {/* Podium */}
              ...
              {/* Summary stats */}
              <div className="px-4 py-4 grid grid-cols-3 gap-3 border-b border-slate-100 dark:border-slate-700">
              ...
              {isDoubles && rankedPairs.length > 0 && (
                <div className="mt-2 border-t border-slate-100 dark:border-slate-700">
                  ...
                  {rankedPairs.map((pair, idx) => (
                    <PairRow key={pair.id} rank={idx + 1} pair={pair} players={session.players} />
                  ))}
                </div>
              )}
                ...
                <div className="px-4 py-3 space-y-4">
                  ...
                  <div>
                    ...
                    {sessionHealth.longestWaiting.length === 0 ? (
                      <p className="text-xs text-slate-400">No one waiting</p>
                    ) : (
                  ...
                  <div>
                    ...
                    {sessionHealth.gamesPlayedSpread.most && sessionHealth.gamesPlayedSpread.least ? (
                      ...
                      <p className="text-xs text-slate-400">No games played</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {sessionHealth.longestMatch ? (
                      ...
                      <p className="text-xs text-slate-400">No completed matches</p>
                    )}
                  </div>
